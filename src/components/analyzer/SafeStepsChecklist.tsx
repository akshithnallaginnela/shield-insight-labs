...
export function SafeStepsChecklist({ steps }: { steps: string[] }) {
  return (
    <section className="glass-card rounded-xl p-5">
      <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Safe Next Steps</h3>
      <ul className="space-y-2">
        {steps.map((s, i) => (
          <li key={i} className="flex items-start gap-3 rounded-lg border border-safe/20 bg-safe/5 p-3">
