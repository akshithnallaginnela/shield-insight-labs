...
export function ShareCard({ result }: { result: AnalysisResult }) {
  ...
  const copyText = async () => {
    const text = `⚠️ ScamShield AI flagged this message as ${tone.label.toUpperCase()} (${result.score}% scam probability).\n\nTop red flags: ${result.redFlags.slice(0, 3).map((f) => f.label).join(", ") || "—"}\n\nVerify before you trust: scamshield.ai`;
    await navigator.clipboard.writeText(text);
    setCopied(true);
    setTimeout(() => setCopied(false), 1800);
  };

  const preview = result.originalText.length > 140 ? result.originalText.slice(0, 140) + "…" : result.originalText;

  return (
    <section className="glass-card rounded-xl p-5">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Share this warning</h3>
        <div className="flex gap-2">
          <button onClick={copyText} className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-white/70 px-3 py-1.5 text-xs hover:bg-white">
            {copied ? <Check className="h-3.5 w-3.5 text-safe" /> : <Share2 className="h-3.5 w-3.5" />}
            {copied ? "Copied!" : "Copy text"}
          </button>
          <button onClick={download} className="inline-flex items-center gap-1.5 rounded-lg bg-gradient-brand px-3 py-1.5 text-xs font-semibold text-white">
      ...
      <div className="overflow-hidden rounded-xl border border-border">
        <svg ref={svgRef} viewBox="0 0 1200 630" className="block h-auto w-full" xmlns="http://www.w3.org/2000/svg">
          <defs>
          ...
          <g transform="translate(60, 60)">
            ...
            <text x="56" y="30" fontFamily="Sora, sans-serif" fontWeight="700" fontSize="24" fill="#0f172a">ScamShield AI</text>
          </g>
          ...
          <g transform="translate(60, 140)">
            ...
            <text x="42" y="29" fontFamily="Manrope, sans-serif" fontWeight="700" fontSize="16" fill={tone.bg}>{tone.label.toUpperCase()}</text>
          </g>

          {/* Score */}
          <g transform="translate(60, 215)">
            <text fontFamily="Sora, sans-serif" fontWeight="800" fontSize="120" fill="#0f172a">{result.score}<tspan fontSize="56" fill="#64748b">%</tspan></text>
            <text y="155" fontFamily="Manrope, sans-serif" fontSize="22" fill="#64748b">scam probability</text>
          </g>
          ...
          <g transform="translate(60, 470)">
            {result.redFlags.slice(0, 3).map((f, i) => (
              <g key={f.id} transform={`translate(${i * 360}, 0)`}>
                ...
                <text x="40" y="32" fontFamily="Manrope, sans-serif" fontWeight="600" fontSize="16" fill="#0f172a">{f.label}</text>
              </g>
            ))}
          </g>
